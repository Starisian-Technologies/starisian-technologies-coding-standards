...
- **Workflow architecture — DOCUMENTED (resolved 2026-06-16).** The full centralized enforcement architecture (reusable workflows, event dispatch, repo-local templates) is documented in [`docs/workflow-strategy.md`](workflow-strategy.md). Two new active workflows added: `adr-dispatch.yml` (sender) and `adr-conformance.yml` (reusable receiver logic). Templates for consuming repos are in `workflow-templates/`.
