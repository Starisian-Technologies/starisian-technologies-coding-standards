...
- Draft new reusable workflows enforcing existing standards
- Update tool configs to match updated standards
- Update the enforcement matrix to reflect new workflows
- Update the catalog when new standards are added
- Fix inconsistencies between prose standards and tool configs

## What agents must NOT do

- Invent standards without an ADR source. If no ADR exists for a rule,
  file an open question — do not create the standard.
- Add product names anywhere in this repo.
- Edit governance snapshot files under `.github/instructions/governance/`.
  These are auto-synced and read-only.
- Mark a matrix row ENFORCED without a working workflow.
- Create repo-local ADRs. This repo has no decision records — decisions
  live in the ADR registry.
- Change the scope of a standard beyond what its source ADR authorizes.

## Naming rules

- This is an organization repo. The company name (Starisian Technologies)
  may appear. Product names may not.
- Standard document filenames use kebab-case.
- Workflow filenames use kebab-case.
- Rule IDs in the matrix use UPPER-SNAKE (e.g. PKG-001, PHP-001).

## Governance reference

Read `.github/instructions/governance/` for current ADR decisions,
invariants, and open questions. These are the rules you enforce.
Do not assume rules that are not in the governance reference.
